...
def register_banking_prompts(mcp) -> None:
    ...
    def categorize_transaction(description: str, direction: str = "auto", top_k: int = 3) -> str:
        return (
            ...
            "from your own knowledge. For an enum-constrained second "
            "opinion you may read the `banking://transaction-categories/codes` "
            "resource (full list of valid codes with hierarchical paths) "
            "and pick the single best fit; if nothing fits, leave it "
            "unclassified. Never emit a code that is not in that list.\n"
